...
"""
import numpy as np
from src.util.moipSol import BaseSol
from src.util.cwmoipSol import CwmoipSol
from src.util.normalConstraint import UtopiaPlane, SolRep
...
class NcgopSol(CwmoipSol):
    ...
    def __init__(self, moipProblem, pk_size):
    ...
    def execute(self):
        ...
        self.pGenerator = SolRep(
            self.utopiaPlane.y_up, self.varNv, self.PKSize)
        objMatrix = np.array(self.moipProblem.attributeMatrix)
        self.pGenerator.setParas(
            objMatrix, self.origi_A, self.origi_B,
            self.origi_Aeq, self.origi_Beq
        )
        points = self.pGenerator.calculate()
        #  print("pGenerator done.")

        # for utopiaPlane calculation, to assure the completeness of resolving
        # we cannot set timeout for this.cplex.
        # then for NCPDG, we can give the timeout
        # for intlinprog timeout for each execution
        ...
        elif self.varNv < 2000:
            self.solver.parameters.timelimit.set(SolRep.Small_TimeOut)
            self.solver.parameters.dettimelimit.set(SolRep.Small_DeterTimeOut)
        else:
            self.solver.parameters.workmem.set(SolRep.WorkMem)
            self.solver.parameters.timelimit.set(SolRep.Big_TimeOut)
            self.solver.parameters.dettimelimit.set(SolRep.Big_DeterTimeOut)
        ...
        for p_k in points:
            ...
            self.calculate(p_k, self.utopiaPlane.y_up,
                           self.utopiaPlane.y_ub, self.utopiaPlane.y_lb)
    ...
    def calculate(self, p_k, y_up, y_ub, y_lb):
        ...
        (rstObj, rstXvar, rstStatusString) = NcgopSol.intlinprog(
            self.solver, self.xvar, ff, extra_A1, extra_B1, [], [], lb, ub)
        ...
        if(rstStatusString.find("optimal") >= 0):
            ...
            cplexResults = CplexSolResult(
                rstXvar, rstStatusString, self.moipProblem)
            self.addTocplexSolutionSetMap(cplexResults)
            fCWMOIP = np.dot(rstXvar, ff)
        return fCWMOIP

    @classmethod
    def intlinprog(cls, cplex, xVar, ff, extra_A1, extra_B1,
                   extra_Aeq1, extra_Beq1, lbs, ubs):
        origiCplex = cplex
        if cplex is None:
            try:
                cplex = Cplex()
        ...
        if xVar is None:
            ...
            var_types = 'B' * (len(ff))
            cplex.variables.add(
                lb=lbs, ub=ubs, types=var_types, names=var_names)
        ...
        cplex.linear_constraints.add(
            lin_expr=inEquationRows, senses='L'*len(extra_A1),
            rhs=extra_B1, names=tempConstNames1
        )
        ...
        cplex.linear_constraints.add(
            lin_expr=equationRows, senses='E'*len(extra_Aeq1),
            rhs=extra_Beq1, names=tempConstNames2)
        ...
        if(rsltSolString.find("optimal") >= 0):
            # bug fixed here, rsltSol should not be returned
            # as the constraints will be modified at the end of the method
            # rsltSol = self.solver.solution
            rsltXvar = cplex.solution.get_values()
            rsltObj = cplex.solution.get_objective_value()

        if origiCplex is None:
            cplex.end()
        else:
