...
class BaseSol:
    ...
    """
    def prepare(self):
        assert len(self.moipProblem.sparseInequationsMapList)==len(self.moipProblem.sparseInequationSensesList)
        ...
        for ineqlDic in self.moipProblem.sparseInequationsMapList:
            ...
            indices = self.solver.linear_constraints.add(lin_expr = rows, senses =  self.moipProblem.sparseInequationSensesList[constCounter-1], rhs = [rs], names = [constName] )
